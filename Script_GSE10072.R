...
#Analisis ekspresi gen bertujuan untuk membandingkan tingkat ekspre7si gen 
...
Hasil_GSE10072_DEG.csv
deg <- read.csv("Hasil_GSE10072_DEG.csv")
View(deg)
genes <- unique(topTableResults$SYMBOL)
genes <- na.omit(genes)
write.table(genes, "gene_list_GO.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)

deg_sig <- deg[deg$adj.P.Val < 0.05 & abs(deg$logFC) > 1, ]
write.csv(deg_sig, "DEG_significant.csv", row.names = FALSE)

deg_final <- deg_sig[,c("SYMBOL","logFC","adj.P.Val","P.Value","GENENAME")]
write.csv(deg_final, "DEG_final_laporan.csv", row.names = FALSE)

write.csv(deg, "DEG_GSE10072.csv", row.names = FALSE)
